          ...
          <div className="col-span-5 border rounded p-4 flex flex-col h-[600px]">
            ...
            {!isReadOnly && (
              <div className="mt-2 flex gap-2 items-center">
                ...
                />
                <Button className="bg-red-500 hover:bg-red-800" onClick={handleSendComment}>Enviar</Button>
              </div>
            )}
          </div>
        </div>
...
export const AgreementActionsDirector = ({
  ...
  const { approveAgreement, rejectAgreement, uploadNewVersion } = useAgreement(agreement.id);
  ...
  const handleUploadNewVersionClick = () => fileInputRef.current?.click();

  const handleNewVersionChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (!file) return;
    ...
    try {
      ...
      toast.success("Nueva versión del convenio subida correctamente.");
      onUpdate();
    } catch {
      toast.error("Error al subir la nueva versión del convenio.");
    } finally {
  ...
  return (
    <div className="flex flex-col gap-3 mt-3 border-t pt-4">
      ...
      <div className="flex gap-2 flex-wrap">
        <Button
          className="bg-zinc-600 hover:bg-zinc-700 text-white px-4 py-2 rounded-md"
